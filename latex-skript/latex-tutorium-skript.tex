...
% Einfügen einer Titelfolie vor jeder neuen Section
\AtBeginSection[]{
	\begin{frame}%{\insertsectionhead}
	\vfil
	\centering
	{\huge\usebeamercolor[fg]{title}\bfseries\insertsectionhead}
	%\tableofcontents[subsectionstyle=show/show/hide, sectionstyle=hide/hide]
	\vfil
	\end{frame}
}
...
Eine Umgebung (\texttt{environment}) muss immer geöffnet \textbf{und} anschließend \textbf{geschlossen} werden. \par\medskip
In die Umgebung kommt dann der Inhalt, den sie beeinflusst. \par\medskip
\begin{lstlisting}
...
\begin{frame}[fragile]{Dokumente kompilieren}
...
\textbf{Jedes} \LaTeX{}-Dokument muss \textbf{mindestens} aus folgenden Befehlen bestehen:
\begin{lstlisting}
\documentclass{article}
...
\begin{itemize}
	\item Präambel ist ähnlich einer Header-Datei bei C
	\item befindet sich zwischen \lstinline|\documentclass[]{}| und \lstinline|\begin{document}|
	\item beinhaltet alle Pakete, die benötigt werden und konfiguriert das gesamte Dokument
...
Abschnitte strukturieren das Dokument und werden \textbf{automatisch} ins Inhaltsverzeichnis eingefügt.\par\medskip
Es gibt \textbf{verschiedene} Tiefen von Abschnitten in folgender Reihenfolge (höchste zu kleinste):
\begin{lstlisting}
%\chapter{}
...
In \LaTeX{} werden alle Verzeichnisse \textbf{automatisch} beim Kompilieren erstellt. \par\medskip
Dazu müssen folgende Befehle an der gewünschten Stelle eingefügt werden:
\begin{lstlisting}
\maketitle			% oder \titlepage erzeugt Titelseite
...
\LaTeX{} unterstützt von Haus aus \textbf{nur} \texttt{.eps}-Dateien. \par\medskip
Mit dem Paket \lstinline|\usepackage{graphicx}| können aber auch \texttt{.jpg}-, \texttt{.png}- und \texttt{.pdf}-Dateien eingebunden werden. \par\medskip
\begin{lstlisting}
\includegraphics[width=\textwidth]{images/Bild.png}
% oder
\includegraphics[scale=0.5]{images/Bild.png}
\end{lstlisting}
Bei der Suche nach Bildern für eine Arbeit sollten \textbf{möglichst hochauflösende} \texttt{jpg}- oder wenn möglich sogar \texttt{pdf}-Dateien verwendet werden.
\end{frame}


\subsection{Abbildungen}
\begin{frame}[fragile]{Abbildungen}
Damit noch eine Bildunterschrift eingefügt und später im Text referenziert werden kann, muss die \texttt{figure}-Umgebung benutzt werden: \par\medskip
\begin{minipage}{0.55\textwidth}
\begin{lstlisting}
\begin{figure}[platzierung]
	\centering	% Zentriert die Abbildung
		\includegraphics{images/harold_stock.jpg}
...
\begin{figure}
	\centering
		\includegraphics[width=0.6\textwidth]{images/harold_stock.jpg}
...
Folgende \textbf{Nomenklatur} wird häufig verwendet: \\
\begin{lstlisting}
...
Es gibt drei verschiedene Listenumgebungen:
	\begin{itemize}
		\item \texttt{itemize}: Stichpunktform
		\item \texttt{enumerate}: Aufzählungsform
		\item \texttt{description}: Liste mit Beschreibungen
	\end{itemize}
\begin{columns}
...
Durch verschiedene Größen, können z.B. Überschriften vom restlichen Text abgesetzt werden.\par\medskip
\begin{tabular}{ll}
...
Text kann folgendermaßen \textbf{zentriert} werden: \par\medskip
...
\end{center} 
	\end{minipage}
Für \textbf{linksbündigen} Text kann \lstinline|\begin{flushleft} ... \end{flushleft}|, \par\medskip
für \textbf{rechtsbündigen} Text \lstinline|\begin{flushright} ... \end{flushright}| bzw. \lstinline|{\raggedright}| verwendet werden.
\end{frame}
...
\LaTeX{} unterscheidet zwei \textbf{Arten der Formeleingabe}:
\begin{itemize}
	\item \textbf{Inline}: innheralb einer Zeile im Text
	\item \textbf{Display}: als eigene Gleichung vom Text getrennt
...
Die \texttt{align}-Umgebung sorgt dafür, dass mehrere Gleichungen \textbf{übereinander} an einem Zeichen angeordnet werden können. Dazu wird das \texttt{\&}-Zeichen verwendet. \par\medskip
\begin{columns}
	\begin{column}{0.48\textwidth}
\begin{lstlisting}
\begin{align}
...
Es gibt \textit{sehr} viele Symbole in \LaTeX{}. \par\medskip
Folgende Symbole können \textbf{direkt} eingegeben werden: \par\medskip
\verb#+ - = ! / ( ) [ ] < > | ' :# \par\medskip
Es ist nützlich ein \textit{Cheat-Sheet} zu benutzen oder die gewünschten Befehle einfach zu googeln.
Im \href{https://en.wikibooks.org/wiki/LaTeX/Mathematics}{Wikibooks} (oder \href{https://en.wikipedia.org/wiki/List_of_mathematical_symbols_by_subject}{Wikipedia}) ist eine Liste an Symbolen zu finden. \par\medskip
Über \href{http://detexify.kirelabs.org/classify.html}{Detexify} kann ein bestimmtes Symbol auch über Zeichnen gefunden werden.
\end{frame}


\subsection{Hoch- \& Tiefstellung}
\begin{frame}[fragile]{Hoch- \& Tiefstellung}
Ausdrücke können mit \verb|^| \textbf{hoch}- und mit \verb|_| \textbf{tiefgestellt} werden. \\
...
Mithilfe des \texttt{siunitx}-Pakets können SI-Einheiten und Zahlen im korrekten Format eingegeben werden. Die Benutzung ist \href{https://www.namsu.de/Extra/pakete/Siunitx.html}{hier} weiter erklärt. \par\medskip
...
\si{\volt\second\per\ampere} % nur Einheit

\SI{100}{\milli\volt} % Einheit mit Zahl
...
\num{123,45} \\ \vfil
\num{123e45} \\ \vfil
\si{\volt\second\per\ampere} \\ \vfil
\SI{100}{\milli\volt} \\ \vfil
\SIlist{1;3;8}{\newton} \\ \vfil
\SIrange{1}{10}{\newton} \\ \vfil
\end{minipage}
\end{frame}
...
%Mit dem \texttt{glossaries}-Paket können Glossare, Indexverzeichnisse und Abkürzungsverzeichnisse erzeugt werden. Eine neue Abkürzung kann mit \lstinline|\newacronym{kürzel}{kurzform}{langform}| erzeugt werden. \par\medskip
%Es ist gut, die Einträge in eine extra Datei auszulagern (z.B. \texttt{abkuerzungen.tex}) \par\medskip
%Um eine Abkürzung zu benutzen, nimmt man den Befehl \lstinline|\gls{kürzel}|. Wenn die Abkürzung das erste mal verwendet wird, wird automatisch sowohl die Kurz- als auch die Langform ausgegeben. \par\medskip
%Das Abkürzungsverzeichnis wird mit \lstinline|\printglossaries| erzeugt.
%\end{frame}

\begin{frame}[fragile]{Abkürzungen}
Mit dem \texttt{nomencl}-Paket können Abkürzungsverzeichnisse erstellt werden. \par\medskip
Eine neue Abkürzung kann mit \lstinline|\nomenclature[sortierzeichen]{abkürzung}{langform}| erzeugt werden. \par\medskip
Leider ist mit diesem Paket keine Referenzierung möglich, dafür gibt es \texttt{glossaries}.
\end{frame}


\subsection{Fußnoten}
\begin{frame}[fragile]{Fußnoten}
Fußnoten \footnote{Dies ist eine Fußnote} können sehr einfach über \lstinline|\footnote{Dies ist eine Fußnote}| erzeugt werden. \par\medskip
Diese können auch verlinkt werden, aber das macht meistens keinen Sinn, denn sie treten sowieso nur innerhalb einer Seite auf.
\end{frame}


\subsection{Quellcode \& Verbatim}
\begin{frame}[fragile]{Verbatim}
Manchmal muss man in \LaTeX{} Zeichen eingeben, die aber bereits reserviert sind. Für einzelne Zeichen ist dies wie \hyperref[sub:reserved]{hier} gezeigt möglich. \par\medskip
Das wird allerdings schnell nervig, wenn z.B. Quellcode dargestellt werden soll. \par\medskip
Eine Möglichkeit Text ''wörtlich'' (engl.: \textit{verbatim}) darzustellen, ist die \texttt{verbatim}-Umgebung:
\lstinline|\begin{verbatim} ... \end{verbatim}| \par\medskip
oder die Inline-Variante: \lstinline|\verb# #| \par\medskip
Bei der Inline Variante ist zu beachten, dass das Anfangs- \& Endzeichen gleich sein müssen und nicht im Text enthalten sein dürfen. \par\medskip
Das \texttt{verbatim}-Paket muss hierzu eingebunden werden!
\end{frame}


\begin{frame}[fragile]{Quellcode}
Bei \texttt{verbatim} wird die Syntax des Quellcodes nicht hervorgehoben. Dafür gibt es jedoch das \texttt{listings}-Paket. \par\medskip
\lstinline|\begin{lstlisting} ... \end{lstlisting}| \par\medskip
oder die Inline-Variante: \lstinline|\lstinline# #| \par\medskip
Die gleichen Regeln wie bei \texttt{verbatim} gelten auch hier. \par\medskip
Die Sprache kann auch global in der Präambel (\lstinline|\lstset{language=...}|) festgelegt werden.
\end{frame}
...
\begin{itemize}
	...
	\item Deutsches \LaTeX{}-Wiki: \href{https://golatex.de/wiki/index.php/Hauptseite}{GoLaTeX}
	\item Mathematische Symbole: \href{https://en.wikibooks.org/wiki/LaTeX/Mathematics}{Wikibooks}
...
%\begin{tabular}{ll}
%\lstinline|\label{}| & Erstellung von Labeln zur Referenzierung \\
%\lstinline|\ref{}| & Ein Label referenzieren \\
%\lstinline|\pageref{}| & Gibt Seitenzahl zu einer Referenz aus \\
%\lstinline|\cite{}| & Quelle zitieren \\
%\lstinline|\cite{S. x}| & Quellenzitation mit Angabe einer \\ & Seitenzahl ''x'' einfügen \\
%\lstinline|\footnote{}| & Fußnote einfügen \\
%\lstinline|$ $| & Formeleingabe \\
%\lstinline|\nomenclature{a.}{ab}| & Abkürzung a. für ab in das \\ & Abkürzungsverzeichnis aufnehmen \\
%\lstinline|\clearpage| & Umbruch auf neue Seite
%\end{tabular}
%\end{frame}
...
%\begin{tabular}{ll}
%\texttt{setspace} &  Verändert den Standardmäßigen Zeilenabstand \\
%\texttt{amsmath, amssymb} & Erweiterung für den Mathematik-Satz 
%\end{tabular}
%\end{frame}
...
\Huge
\vfill
\textbf{\textit{Vielen Dank für}} \\ \vfill
\textbf{\textit{Ihre}} \\ \vfill
\textbf{\textit{Aufmerksamkeit!}} \\ \vfill
\end{center}
\end{frame}
